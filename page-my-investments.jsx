      ...
      <div style={{ display: 'flex' }}>
        ...
        <div style={{ flex: 1, padding: '32px 56px 0 0' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: T.gap }}>
            {items.map((it, i) => (
              <div key={i} style={{ background: T.card, borderRadius: T.rLg, padding: 28, display: 'grid', gridTemplateColumns: '1.5fr 1fr 1fr 1fr 100px', gap: 24, alignItems: 'center' }}>
                <div>
                  <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                    <Tag>{it.tag}</Tag>
                    <Tag tone={it.status === '진행중' ? 'green' : it.status === '연체' ? 'neutral' : 'neutral'}>{it.status}</Tag>
                  </div>
                ...
                <div>
                  ...
                  <div style={{ height: 4, background: T.bg, borderRadius: T.rPill, overflow: 'hidden' }}>
                    <div style={{ width: pct(it.progress), height: '100%', background: it.status === '연체' ? '#c4452f' : T.ink }}></div>
                  </div>
                  <div style={{ fontSize: 11, color: T.ink2, marginTop: 6 }}>다음 {it.nextDate}{it.nextAmt > 0 ? ` · +${it.nextAmt.toLocaleString()}원` : ''}</div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <span style={{ fontSize: 13, color: T.ink, fontWeight: 500 }}>상세 →</span>
                </div>
              </div>
            ))}
          </div>
...
window.PageMyInvestments = PageMyInvestments;
